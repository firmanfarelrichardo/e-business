...
<x-ui.glass-card variant="light"
    class="group flex flex-col h-full bg-white/80 hover:bg-white p-4 cursor-pointer relative overflow-hidden">
    <!-- Badge -->
    @if($badge)
    ...
    <!-- Image -->
    <div
        class="aspect-square mb-4 flex items-center justify-center p-4 bg-white rounded-2xl border border-slate-100 group-hover:border-brand-primary/30 transition-colors">
        @if($image)
    ...
    <div class="flex-grow flex flex-col">
        ...
        <div class="mt-auto">
            @if($originalPrice)
                <div class="text-xs text-slate-400 line-through mb-0.5">Rp {{ number_format($originalPrice, 0, ',', '.') }}
                </div>
            @endif
    ...
    <div
        class="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-in-out bg-gradient-to-t from-white via-white to-transparent">
        <form action="{{ url('/cart/add') }}" method="POST">
            ...
            <button type="submit"
                class="w-full bg-brand-dark hover:bg-brand-primary text-white text-sm font-semibold py-2 rounded-xl shadow-md transition-colors flex items-center justify-center gap-2">
