    ...
    <div class="bg-[#2C3437] rounded-b-xl px-2 shadow-sm border border-slate-200">
        <div class="flex overflow-x-auto hide-scrollbar">
            @foreach($subcategories as $sub)
                <a href="{{ url('/not-configured') }}" class="whitespace-nowrap px-6 py-2.5 text-xs font-semibold text-white/80 hover:text-white hover:bg-white/10 rounded-lg transition-colors my-1 mx-1">
                    {{ $sub }}
                </a>
            @endforeach
        </div>
