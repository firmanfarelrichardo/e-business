...
<body class="text-slate-800 antialiased flex flex-col min-h-screen relative overflow-x-hidden">
    ...
    <div class="fixed w-full z-50 top-0 px-0 sm:top-4 sm:px-4 print:hidden">
        <nav
            ...
            <a href="/" class="flex items-center gap-3 group z-10">
                ...
                <span
                    class="font-bold text-xl tracking-tight text-brand-dark group-hover:text-brand-primary transition">Sinergi.</span>
            </a>

            <!-- Desktop Navigation -->
            <div class="hidden md:flex space-x-8 items-center z-10">
                <a href="{{ url('/katalog') }}"
                    class="text-slate-700 hover:text-brand-primary text-sm font-semibold transition duration-200">Katalog
                    ATK</a>
                <a href="{{ url('/jasa') }}"
            ...
            <div class="flex items-center space-x-4 z-10">
                ...
                <a href="{{ url('/keranjang') }}"
                    class="relative p-2 text-slate-700 hover:text-brand-primary transition duration-200">
                    ...
                    <div class="hidden md:flex items-center space-x-3">
                        @if(auth()->user()->role === 'member')
                            <a href="{{ route('profile') }}"
                                class="text-slate-700 hover:text-brand-primary font-medium px-2 text-sm">Profil Saya</a>
                        @else
                            <a href="{{ url('/dashboard') }}"
                                class="text-slate-700 hover:text-brand-primary font-medium px-2 text-sm">Dashboard</a>
                        @endif
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="m-0">
                            @csrf
                            <button type="submit"
                                class="text-red-500 hover:text-red-700 font-medium px-2 text-sm bg-transparent border-none focus:outline-none cursor-pointer">
                                Keluar
                            </button>
                        </form>
                    </div>
                @else
                    <a href="{{ url('/login') }}"
                        class="hidden md:block text-slate-700 hover:text-brand-primary text-sm font-medium transition duration-200 px-2">Masuk</a>
                @endauth
    ...
    <main class="flex-grow pt-20 sm:pt-28 pb-12 relative z-10">
        ...
        <div class="blob bg-brand-tertiary w-[30rem] h-[30rem] top-20 -left-20 mix-blend-multiply opacity-50 -z-10">
        </div>
        <div class="blob bg-brand-secondary w-[20rem] h-[20rem] bottom-10 right-10 mix-blend-multiply opacity-40 -z-10"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-8">
                ...
                <div class="lg:col-span-1 border-r border-white/10 pr-6">
                    <a href="/" class="flex items-center gap-2 mb-4 group inline-flex">
                        <div
                            class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center border border-white/20 group-hover:bg-brand-primary transition">
    ...
    <!-- Global Scripts/Animations for all App features (Katalog, Jasa, Profile-Member) -->
    <script type="module">
