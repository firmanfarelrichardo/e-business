...
@section('content')
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-800 tracking-tight motion-title">Manajemen Jasa & Layanan</h1>
        <p class="text-sm text-gray-500 motion-title mt-1">Kelola data servis percetakan dan harga satuan</p>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/3">Layanan</th>
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/3">Deskripsi</th>
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider lg:w-1/6">Harga / Pcs</th>
                        <th class="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right lg:w-1/6">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    @forelse($services as $service)
                        <tr class="hover:bg-gray-50/50 transition motion-table-row service-row" data-name="{{ strtolower($service->name) }}">
                            <td class="py-4 px-6">
                                <div class="flex items-center gap-4">
                                    @if(!empty($service->attachments) && is_array($service->attachments) && count($service->attachments) > 0)
                                        <img src="{{ Storage::url($service->attachments[0]) }}" alt="{{ $service->name }}" class="w-10 h-10 rounded-lg object-cover border border-gray-200 shrink-0">
                                    @else
                                        <div class="w-10 h-10 rounded-lg bg-[#7B9B6F]/10 text-[#7B9B6F] flex items-center justify-center shrink-0">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <td class="py-4 px-6">
                                <div class="text-xs text-gray-500 line-clamp-2 max-w-sm">{{ $service->description ?? '-' }}</div>
                            </td>
                            <td class="py-4 px-6">
                                <div class="text-sm font-semibold text-gray-800">Rp {{ number_format($service->piece_price, 0, ',', '.') }}</div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-12 text-center text-gray-400">Belum ada data jasa/layanan di sistem.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="modal-create-service" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-6 border-b border-gray-100">
                ...
                <button onclick="closeServiceCreateModal()" class="text-gray-400 hover:text-gray-600 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form action="{{ route('dashboard.services.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Nama Layanan <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Harga Per Pcs (Rp) <span class="text-red-500">*</span></label>
                    <input type="number" name="piece_price" required min="0" placeholder="5000"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#7B9B6F] transition">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Foto Layanan <span class="text-[10px] text-gray-400 font-normal">(bisa banyak)</span></label>
    ...
    <div id="modal-edit-service" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-6 border-b border-gray-100">
                <h2 class="text-lg font-bold text-gray-800">Edit Jasa</h2>
                <button onclick="closeServiceEditModal()" class="text-gray-400 hover:text-gray-600 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <form id="edit-service-form" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Tambah Foto <span class="text-[10px] text-gray-400 font-normal">(opsional)</span></label>
                    <input type="file" name="attachments[]" multiple accept="image/*"
                        class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#E8F0E5] file:text-[#5A6852] hover:file:bg-[#D5E1D1] transition cursor-pointer">
                </div>
                <div class="flex gap-3 pt-2">
